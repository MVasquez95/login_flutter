...
class Prefs{
  ...
  static Future<bool> removeAuth() async {
    var instance = await _prefs;
    bool userKey = await instance?.remove(AppHelpers.USER_KEY_PREFS);
    bool tokenKey = await instance?.remove(AppHelpers.TOKEN_KEY_PREFS);
    bool addressKey = await instance?.remove(AppHelpers.ADDRESS_KEY_PREFS);
    return userKey && tokenKey && addressKey ?? Future.value(false);
  }

  static Future<bool> removeRemember() async {
    var instance = await _prefs;
    bool emailKey = await instance?.remove(AppHelpers.EMAIL_KEY_PREFS);
    bool passwordKey = await instance?.remove(AppHelpers.PASSWORD_KEY_PREFS);
    return emailKey && passwordKey ?? Future.value(false);
  }
